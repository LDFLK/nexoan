...
func (repo *Neo4jRepository) GetGraphEntity(
	ctx context.Context,
	entityId string,
) (*pb.Kind, *pb.TimeBasedValue, string, string, error) {
...
func (repo *Neo4jRepository) GetGraphRelationships(
	ctx context.Context,
	entityId string,
) (map[string]*pb.Relationship, error) {
...
func (repo *Neo4jRepository) GetFilteredRelationships(
	ctx context.Context,
	entityId string,
	...
	if err != nil {
		log.Printf(
			"[GetEntityIdsByRelationship] Error fetching related relationships for entity %s with filters %v: %v",
			entityId,
			filters,
			err,
		)
...
func validateGraphEntityCreation(entity *pb.Entity) bool {
	...
	if entity.Kind == nil || entity.Kind.GetMajor() == "" || entity.Kind.GetMinor() == "" {
		log.Printf(
			"[neo4j_handler.validateGraphEntityCreation] Skipping Neo4j entity creation for %s: Missing or empty Kind.Major",
			entity.Id,
		)
		return false
	}
	...
	if entity.Name == nil || entity.Name.GetValue() == nil {
		log.Printf(
			"[neo4j_handler.validateGraphEntityCreation] Skipping Neo4j entity creation for %s: Missing or empty Name.Value",
			entity.Id,
		)
		return false
	}
	...
	if entity.Created == "" {
		log.Printf(
			"[neo4j_handler.validateGraphEntityCreation] Skipping Neo4j entity creation for %s: Missing Created date",
			entity.Id,
		)
		return false
	}
...
func (repo *Neo4jRepository) HandleGraphEntityCreation(ctx context.Context, entity *pb.Entity) (bool, error) {
	...
	if !validateGraphEntityCreation(entity) {
		...
		return false, fmt.Errorf(
			"[neo4j_handler.HandleGraphEntityCreation] missing required fields for Neo4j entity creation",
		)
	}
	...
	if entity.Kind == nil || entity.Kind.GetMajor() == "" || entity.Kind.GetMinor() == "" {
		return false, fmt.Errorf(
			"[neo4j_handler.HandleGraphEntityCreation] missing or invalid Kind.Major or Kind.Minor for entity %s",
			entity.Id,
		)
	}
...
func (repo *Neo4jRepository) HandleGraphEntityUpdate(ctx context.Context, entity *pb.Entity) (bool, error) {
	...
	if entity.Name != nil && entity.Name.GetValue() != nil {
		...
		if err != nil {
			log.Printf(
				"[neo4j_handler.HandleGraphEntityUpdate] Error unpacking Name value for entity %s: %v",
				entity.Id,
				err,
			)
...
func (repo *Neo4jRepository) HandleGraphRelationshipsCreate(ctx context.Context, entity *pb.Entity) error {
	if len(entity.Relationships) == 0 {
		log.Printf(
			"[neo4j_handler.HandleGraphRelationshipsCreate] No relationships to process for entity: %s",
			entity.Id,
		)
		return nil
	}
	...
	log.Printf(
		"[neo4j_handler.HandleGraphRelationshipsCreate] Processing %d relationships for entity: %s",
	...
	for _, relationship := range entity.Relationships {
		...
		if relationship.RelatedEntityId == "" {
			log.Printf(
				"[neo4j_handler.HandleGraphRelationshipsUpdate] Missing RelatedEntityId for relationship creation",
			)
		...
		if err != nil || childEntityMap == nil {
			log.Printf(
				"[neo4j_handler.HandleGraphRelationshipsCreate] Child entity %s does not exist in Neo4j. Make sure to create it first.",
				relationship.RelatedEntityId,
			)
			return fmt.Errorf(
				"[neo4j_handler.HandleGraphRelationshipsCreate] child entity %s does not exist",
				relationship.RelatedEntityId,
			)
		}
		log.Printf(
			"[neo4j_handler.HandleGraphRelationshipsCreate] Child entity %s exists in Neo4j",
			relationship.RelatedEntityId,
		)
...
func (repo *Neo4jRepository) HandleGraphRelationshipsUpdate(ctx context.Context, entity *pb.Entity) error {
	...
	if len(entity.Relationships) == 0 {
		log.Printf(
			"[neo4j_handler.HandleGraphRelationshipsUpdate] No relationships to process for entity: %s",
			entity.Id,
		)
		return nil
	}
	...
	log.Printf(
		"[neo4j_handler.HandleGraphRelationshipsUpdate] Processing %d relationships for entity: %s",
		len(entity.Relationships),
		entity.Id,
	)
	...
	for _, relationship := range entity.Relationships {
		...
		if relationshipExists {
			...
			log.Printf(
				"[neo4j_handler.HandleGraphRelationshipsUpdate] Relationship %s exists, updating...",
				relationship.Id,
			)
			...
			if relationship.Name != "" || relationship.RelatedEntityId != "" || relationship.Direction != "" {
				...
				log.Printf(
					"[neo4j_handler.HandleGraphRelationshipsUpdate] Cannot update immutable fields: %v",
					invalidFields,
				)
				return fmt.Errorf(
					"cannot update immutable fields: %v. Only StartTime and EndTime are allowed",
					invalidFields,
				)
			}
			...
			if len(relationshipData) == 0 {
				...
				return fmt.Errorf(
					"no valid fields provided for relationship update. Only StartTime and EndTime are allowed",
				)
			}

			log.Printf(
				"[neo4j_handler.HandleGraphRelationshipsUpdate] Updating relationship with data: %+v",
				relationshipData,
			)
			...
			log.Printf(
				"[neo4j_handler.HandleGraphRelationshipsUpdate] Successfully updated relationship %s",
				relationship.Id,
			)
			continue
...
func (repo *Neo4jRepository) HandleGraphEntityFilter(
	ctx context.Context,
	req *pb.ReadEntityRequest,
